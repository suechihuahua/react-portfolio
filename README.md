...
Everything on the site comes from `src/content/site.js`. Add an object to `pages` and it gets a nav link, a planet, and a route. Replace `public/resume.pdf` with your real résumé — nothing else needs to change. The placeholder currently checked in there was generated by `npm run make:resume`.
